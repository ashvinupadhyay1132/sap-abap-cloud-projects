...
" Local Handler Class containing RAP Lifecycle Logic
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_authorizations.
    " Enterprise Authorization Check Logic
  ...
  METHOD acceptTravel.
    " 1. EML MODIFY to update status to 'A' (Accepted)
    ...
    " 2. Read updated entity for UI result
  ...
  METHOD rejectTravel.
    " 1. EML MODIFY to update status to 'X' (Rejected)
    MODIFY ENTITIES OF zcds_i_travel_m IN LOCAL MODE
      ENTITY Travel
        UPDATE FIELDS ( Overall_Status )
        WITH VALUE #( FOR key IN keys ( %tky           = key-%tky
  ...
  METHOD validateCustomer.
    " Read customer IDs from key list
    READ ENTITIES OF zcds_i_travel_m IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD calculateTotalPrice.
    " Automatically recalculate total price when booking fee or flight prices change
    READ ENTITIES OF zcds_i_travel_m IN LOCAL MODE
      ENTITY Travel
    ...
    LOOP AT lt_travels INTO DATA(ls_travel).
      DATA(lv_calculated_total) = ls_travel-Booking_Fee + 500. " Base fee calculation logic
